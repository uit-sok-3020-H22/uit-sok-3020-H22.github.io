...
#' and this increases the probability that the sample mean will be "close" to the true population parameter
...
1/sqrt(0.25)
2*pnorm(2)-1 # probability  (this is equal to pnorm(2)-pnorm(-2))

#' Let's make a plot of it
N <- 1:80
dta <- as.data.frame(cbind(N, 1/sqrt(10/N), 2*pnorm(1/sqrt(10/N))-1))  #calculate probability for different size length N=1:80
head(dta)
names(dta)[2:3] <- c("Z", "Prob")
head(dta)

xyplot(Prob~N, data = dta, main="The probability that the true mean is within 1 sd as a function of N")
   #the figure shows as N increases, the probability that the sample mean will provide an estimate within 1 inch of the true value, increases and close to 1

#' The above examples were carried out assuming the population we are considering (i.e., hip width of the U.s. adults) has a normal distribution.
#' The question we need to ask is what about if the population is not normally dist. 
#' 
#' Example C.5
...
gf_dens(~xbar)  #gf_dens() is an alternative way used to displays the density plot slightly differently
mean(xbar)
sd(xbar)
#' n=3
xbar2=numeric(10000)
...
skewness(hip)  #the hip data is slightly negatively skewed 
...
#' Example C.7
#' Our estimates earlier suggest that the hip size of of U.S. adults is normally distributed with ...
#' mean 17.158 inches wide, and with a variance of 3.265. Based on this information, if an airplane seat is 18 inches wide..
#' what % of customers will not be able to fit?
#' 
...
#' Based on our estimates, 32% of the population would not be able to fit into a seat that is 18 inch wide
...
#' Thus, to accommodate 95% of U.S. adult passengers, we estimate that the seats
#' should be slightly greater than 20inches wide
...
#' Example C.8 
#' Simulating the hip data: interval estimates; given mu=10 and known varinace, sigma-square=10, using data from a normal pop
#' Y ~ N(10, 10)
...
value = 10   #pop mean /or true mean 
rmeans %>% 
  mutate(LowerBound=mean-1.96*sqrt(10/30),
         UpperBound=mean+1.96*sqrt(10/30)) %>%
  mutate(ok = value >= LowerBound & value <= UpperBound)  #are the intervals contain the true pop mean, which is equal to 10
                                                          # not all of the interval estimates contain the true value. However, 95% of the intervals constructed 
                                                          # contain the true parameter value. 


#' do 10000 simulations (what about if we do it for 10,000 samples)
...
prop(~ok, data = rmeans10000)   #proportion or probability of True, meaning intervals containing the true value
