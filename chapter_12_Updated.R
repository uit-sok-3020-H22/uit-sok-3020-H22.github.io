...
#' For checking stationary, the usual t-critical values and p-values 
#' cannot be used.
#' Instead we compare the t-critical value for the first lag of 
#' the dep.var. with the critical value from Table 12.2. (see the Text Book) 
#' Reject Ho if tau(t-value) <= t_Cv, (t_Cv=-2.86 at 5% level)
#' Notice here two augmentation terms have been included for 
#' both variables, to account for serial correlation. 
...
#' two augmentation terms minimized the SC, eliminated major 
...
#' Thus, we reject the null hypothesis of non-stationary and
...
plot(diff(ffr.ts))
plot(diff(br.ts))
#' Given their plots fluctuate around zero, we use Dickey-Fuller test
#' equation with no intercept and no trend.
...
#' Cointegration implies the relationship between I(1) variables 
#' such as the residuals are I(0)
...
#-------------------------------------
#' Reject the Ho:residuals are non-stationary(or the series are not co-integrated)
#' if t-value (tau) <=t_c(critical values for co-integration given in Table 12.4, page 583) 
#' The calculated t-value (tau)=-5.526.
#' t-critical = -3.37 
#' Since the t-value <=t_c, Reject H0, and conclude that 
#' residuals are stationary (or the series are co-integrated). 
#---------------------------------------
adf.test(e)
...
#' long-run relationship while a relationship between I(0) variables 
#' is often referred to as a short-run relationship.
#' Error correction model is a dynamic relationship between I(0) variables,
#' which embeds a co-integrating relationship
...
#' Alternatively, 
#' Estimate directly using non-linear least squares or 
